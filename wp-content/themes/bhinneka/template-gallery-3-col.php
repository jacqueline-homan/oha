<?php
/*
Template Name: Gallery Page - 3 columns
*/
get_header(); ?>
				...
				<section class="module">
					...
					<div id="gallery-wrapper">
						...
						<ul id="module-wrapper">
							...
							<li class="<?php foreach ($terms as $term) { echo strtolower(preg_replace('/\s+/', '-', $term->name)). ' '; } ?> gallery_col_three module-box <?php if ( $count % 3 == 0 ) echo' omega';?>" data-id="id-<?php echo $count; ?>">
								<!-- Post begin -->
								<div  id="post-<?php the_ID(); ?>" <?php post_class('gallery_module'); ?> >
									<?php if ( (function_exists('has_post_thumbnail')) && (has_post_thumbnail()) ) { ?>
										<?php bnk_colorbox(get_the_ID(), 'gallery-thumbnail-m'); ?>
									<?php } ?>
								</div>
								<!-- Post end -->
							</li>

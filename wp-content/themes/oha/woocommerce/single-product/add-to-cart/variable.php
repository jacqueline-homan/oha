			...
			<?php $loop = 0; foreach ( $attributes as $name => $options ) : $loop++; ?>
				<tr>
					...
					<td><select id="<?php echo esc_attr( sanitize_title($name) ); ?>" name="attribute_<?php echo sanitize_title($name); ?>">
						<option value=""><?php echo __('Choose an option', 'kauri') ?>&hellip;</option>
						<?php 
							if ( is_array( $options ) ) {
...
f
				...
				<tr>
					...
					<td><?php
						if ( sizeof($attributes) == $loop ) {
							echo '<a class="reset_variations" href="#reset">'.__('Clear selection', 'kauri').'</a>';
						}
					?></td>
				</tr>
	...
	<div class="single_variation_wrap" style="display:none;">
		...
		<div class="variations_button">
			...
			<button type="submit" class="button alt"><?php echo apply_filters('single_add_to_cart_text', __('Add to cart', 'kauri'), $product->product_type); ?></button>
		</div>
	</div>
